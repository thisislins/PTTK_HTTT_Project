...
{
    public partial class QLPhieuKham : UserControl
    {
        public QLPhieuKham()
        {
            InitializeComponent();
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                PhieuKham pk = new PhieuKham(mabs, makh, ngaykham, nhietdo, nhiptim, huyetap, ketluan);
                PhieuKhamBLL.ThemPhieuKham(pk);
                MessageBox.Show("Thêm phiếu khám thành công!");
                reset();
                dgvDSPhieuKham.DataSource = PhieuKhamBLL.LayDSPhieuKham();
        ...
        {
            //DataTable dt = PhieuKhamBLL.LayDSPhieuKham();
            dgvDSPhieuKham.DataSource = PhieuKhamBLL.LayDSPhieuKham();




            cbBS.DataSource = PhieuKhamBLL.LayDSBacSi();
            cbBS.DisplayMember = "MANV";
            cbBS.ValueMember = "MANV";

            cbKH.DataSource = PhieuKhamBLL.LayDSKhachHang();
            cbKH.DisplayMember = "MAKH";
            cbKH.ValueMember = "MAKH";

            /*cbBS.DataSource = dt;
            cbBS.DisplayMember = dt.Columns["PRIVILEGE"].ColumnName;
            cbBS.ValueMember = "MANV";
            ...
            /*cbPrivilege.DataSource = dt;
            cbPrivilege.DisplayMember = dt.Columns["PRIVILEGE"].ColumnName;
            cbPrivilege.ValueMember = dt.Columns["PRIVILEGE"].ColumnName;

            cbTableName.DataSource = dt;
            cbTableName.DisplayMember = dt.Columns["TABLE_NAME"].ColumnName;
            cbTableName.ValueMember = dt.Columns["TABLE_NAME"].ColumnName;*/
        }
